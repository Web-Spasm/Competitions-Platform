...
from App.models import Moderator, Competition, StudentTeam, CompetitionTeam
...
def add_results(mod_name, comp_name, team_name, students, score):
    ...
    comp = Competition.query.filter_by(name=comp_name).first()
    teams = StudentTeam.query.filter_by(team_name=team_name).all()
    ...
    if not mod:
        ...
        if not comp:
            ...
            for team in teams:
                comp_team = CompetitionTeam.query.filter_by(comp_id=comp.id, student_team_id=team.id).first()
...
def update_ratings(mod_name, comp_name):
    ...
    if not mod:
        ...
        for comp_team in comp_teams:
            team = StudentTeam.query.filter_by(id=comp_team.student_team_id).first()
